...
export default function WeeklySummaryPage() {
  ...
  // 今週の収支を取得
  const fetchWeeklySummary = async (date: Dayjs) => {
    setLoading(true)
    try {
      ...
      const incomeData: any[] = incomeResponse.data || []
      const weeklyIncomes = incomeData.filter((item) => {
        if (!item.date) return false
        const incomeDate = dayjs(item.date)
        return (
          incomeDate.isSameOrAfter(weekStart, 'day') && incomeDate.isSameOrBefore(weekEnd, 'day')
        )
      })
      const totalIncome = weeklyIncomes.reduce((sum, item) => sum + (item.amount || 0), 0)
      setIncomeDetails(weeklyIncomes)

      // 支出データ取得
      const expenseResponse = await getPurchasements()
      const expenseData: PurchasementColumn[] = expenseResponse.data || []
      const weeklyExpenses = expenseData.filter((item) => {
      ...
      const totalExpense = weeklyExpenses.reduce((sum, item) => sum + (item.totalPrice || 0), 0)
      setExpenseDetails(weeklyExpenses)

      // 使用記録データ取得
      const consumptionResponse = await getConsumption()
      const consumptionData: ConsumptionColumn[] = consumptionResponse.data || []
      const weeklyConsumptions = consumptionData.filter((item) => {
      ...
      setConsumptionDetails(weeklyConsumptions)
    ...
    <div style={{ padding: '24px', backgroundColor: '#f0f2f5', minHeight: '100vh' }}>
      ...
      <Card style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          ...
          />
          <Tag color="blue">
            {weekStart.format('YYYY年MM月DD日')} ~ {weekEnd.format('YYYY年MM月DD日')}
          </Tag>
        </div>
      </Card>
      ...
      <Spin spinning={loading}>
        ...
        >
          <Table
            ...
            scroll={{ x: 800 }}
            size="small"
            locale={{ emptyText: '今週の収入データはありません' }}
          />
        </Card>
        ...
        >
          <Table
            ...
            scroll={{ x: 800 }}
            size="small"
            locale={{ emptyText: '今週の支出データはありません' }}
          />
        </Card>
        ...
        >
          <Table
            ...
            scroll={{ x: 800 }}
            size="small"
            locale={{ emptyText: '今週の使用記録データはありません' }}
          />
        </Card>
      </Spin>
    </div>
